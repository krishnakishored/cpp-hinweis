...
//A Working MessageQueue Example
/*
the class simply wraps an int that keeps track of the results of the operations and a bool that determines when the calculations are complete. 
These values are manipulated using an overridden += operator, -= operator, and * operator. 
There is also a Finished method that sets the m_Finished Boolean to true.
*/
class RunningTotal
{
  private:
  ...
  public:
    ...
    int operator*() const throw(int) //warning: dynamic exception specifications are not permitted in this mode
